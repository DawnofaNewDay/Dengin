...
public class Player : GameObject
{
    ...
    public override void Move(Vector2f offset)
    {
        ...
        if (hypoPos.X + Size.X > _win.Size.X) offset.X = _win.Size.X - (Pos.X + Size.X);
        if (hypoPos.X < 0) offset.X = -Pos.X;
        if (hypoPos.Y + Size.Y > _win.Size.Y) offset.Y = _win.Size.Y - (Pos.Y + Size.Y);
        if (hypoPos.Y < 0) offset.Y = -Pos.Y;
        base.Move(offset);
    }
}